...
(.venv/lib/python3.13/site-packages/strands_evals/), not its published docs:
  ...
  fix_type check uses substring containment so it also fires on the real
  TOOL_DESCRIPTION_FIX / SYSTEM_PROMPT_FIX values, not just the literal "tool"/"prompt"
  used in the unit tests below.
"""
...
_PERMISOS = ("accessdenied", "unauthorized", "not authorized", "permission", "forbidden")
...
def bucket(location: str, fix_type: str, causality: str) -> str:
    ...
    if re.search(r"\biam\b", text) or any(k in text for k in _PERMISOS):
        return "permisos"
    if "tool" in fix_type.lower() or any(k in text for k in _TOOL):
        return "tool"
...
def render(result) -> str:
    ...
    for rc in result.root_causes:
        ...
        lines.append(f"{rc.location[:28]:<28} {rc.fix_type[:22]:<22} {rc.causality[:16]:<16} | {b}")
